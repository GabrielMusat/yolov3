...
parser.add_argument('--img-width', default=416, type=int, help='Image height')
parser.add_argument(
    '-p',
    '--plot_model',
...
def _main(args):
    config_path = os.path.expanduser(args.cfg)
    weights_path = os.path.expanduser(args.weights)
    assert config_path.endswith('.cfg'), '{} is not a .cfg file'.format(config_path)
    assert weights_path.endswith('.weights') or weights_path.endswith('.pt'), '{} is not a .weights file'.format(weights_path)
    if weights_path.endswith('.pt'):
        model = Darknet(config_path).to('cpu')
        model.load_state_dict(torch.load(weights_path, map_location='cpu')['model'])
        save_weights(model, path='converted.weights', cutoff=-1)
        weights_path = 'converted.weights'
